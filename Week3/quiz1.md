...
- 답: Create a merge commit은 main 외 branch의 커밋들을 그대로 main에 병합하는 것이고,  
Squash and merge는 main 외 branch의 커밋들을 하나의 커밋으로 main에 병합하는 것이고,  
Rebase and merge는 main에 뒤이어 커밋들이 병합된다.
...
- 답: 가장 최근 커밋이 readme 파일을 추가한 것, 현재 branch가 develop인 것, commit id가 a1s2d3f인 것을 알 수 있다. 
...
- 답: reset은 커밋을 삭제하고 다른 branch에 영향을 줄 수 있기 때문에 되돌리는 새 커밋을 만드는 revert를 선택할 것이다.